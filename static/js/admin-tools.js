(function () {
  ...
  document.querySelectorAll('[data-dev-tools-clear-table]').forEach((button) => {
    button.addEventListener('click', async () => {
      ...
      try {
        await postJSON('/admin/tools/clear-table', { table: table });
  ...
  document.querySelectorAll('[data-dev-tools-seed-messages]').forEach((button) => {
    button.addEventListener('click', async () => {
      const result = document.querySelector('[data-dev-tools-seed-result]');
      try {
        const data = await postJSON('/admin/tools/seed-messages', {});
        showMessage(result, 'Created ' + data.created + ' message(s).', false);
      } catch (err) {
        showMessage(result, err.message, true);
      }
    });
  ...
  document.querySelectorAll('[data-dev-tools-execute-sql]').forEach((button) => {
    button.addEventListener('click', async () => {
      ...
      const query = textarea && textarea.value && textarea.value.trim();
      if (!query) return;

      result.innerHTML = '<p class="small text-muted">Running...</p>';
      try {
        const data = await postJSON('/admin/tools/execute-sql', { query: query });
